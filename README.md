...
截至 2026 年 8 月，OpenCLI 1.8.6 的 V2EX 适配器提供 `hot`、`latest`、`node`、`topic`、`replies`、`member` 等只读命令，可以读取热门/最新主题、节点主题、单帖正文和回复；它没有 `search` 子命令。因此，“V2EX 没有关键词搜索接口”基本准确，但“V2EX 没接口、不能采”不准确。
