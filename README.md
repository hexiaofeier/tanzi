...
- **内置搜查令**：覆盖行业、企业、用户研究和舆情热点的关键词设计、对象扩展及反例搜索，无需另装搜查令。默认输出简短搜索任务包，已有计划时直接沿用。
- **增强公众号采集**：加入基于 [wx-search-cli](https://github.com/tjx666/wx-search-cli) 改造的采集脚本，支持关键词搜索、原文链接解析、正文读取，以及从命中文章发现候选公众号；保留上游 MIT 许可及来源说明。
- **补充小红书读取入口**：针对部分导航故障，提供文字正文和少量一级评论的备用读取路径。
- **同步探子主体与使用说明**：更新渠道选择、依赖安装、内容验收和响应脱敏规则。

本次更新通过 15 项离线回归测试，并完成最新版用户研究、舆情热点搜索设计补测。平台实际可用性仍以运行时验证为准。
